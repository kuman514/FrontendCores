...
### 스코프의 작동 방식
- JavaScript는 함수가 실행될 때마다 메모리 상에 실행 컨텍스트를 생성한다. 함수의 실행 컨텍스트는 한경 레코드와 외부 렉시컬 환경 참조를 가지고 있다.
  - 환경 레코드: 현재 스코프에서 선언한 변수와 함수가 저장되어 있는 영역.
  - 외부 렉시컬 환경 참조: 현재 환경에서 변수나 함수를 찾지 못할 경우 외부에서 참조하기 위한 외부 렉시컬 환경도 존재한다.
- 해당 함수나 변수를 찾기 위해 현재 환경 레코드에서 찾아보고, 없을 경우 외부 렉시컬 환경 참조를 통해 변수와 함수릉 찾는다. 만약 모든 상위 스코프를 찾아다녔음에도 찾지 못하면 전역 스코프까지 참조하게 되는데, 여기서조차 찾을 수 없다면 Reference Error를 발생시킨다.
