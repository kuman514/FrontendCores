...
## 이벤트 루프
- 이벤트 루프란
  - 싱글 스레드인 JavaScript가 멀티 스레드인 것처럼 동작하게 만드는 과정.
- 틱 (Tick) 작동 과정
  - 콜 스택이 비어있는지 확인한다.
  - 태스크 큐에 실행할 작업을 확인한다.
...
- 태스크 큐
  ...
  - JavaScript 엔진 외에 있다.
  - 마이크로태스크 큐와 매크로태스크 큐
    - 마이크로태스크 큐는 Promise.then의 콜백이나 async 안 await 이후의 로직 등등이 들어가는 곳이다.
    - 매크로태스크 큐는 setTimeout이나 setInterval 등등의 콜백이 들어가는 곳이다.
    - 마이크로태스크 큐가 먼저 비워진 뒤, 매크로태스크 큐를 확인한다.
...
## this가 정해지는 우선순위
1. new 객체 생성을 통해 생성되었는가? (new 바인딩)
2. .call, .bind, .apply의 thisArg 파라미터로 전달받았는가? (명시적 바인딩)
3. 객체의 멤버로써 호출되었는가? (암묵적 바인딩)
4. 일반적인 호출인가? (기본 바인딩)
  - 이 때 this는 전역(브라우저에선 window, Node에선 global)로 결정된다.
- 1번 ~ 4번 조건 중 여러 개를 충족한다면, 1번-2번-3번-4번 순서의 우선순위로 this가 결정된다.
- 화살표 함수는 this를 동적으로 바인딩하지 않고, 무조건 화살표 함수가 선언된 렉시컬 스코프가 this로 정해진다.
...
## 클로저
- 자신의 렉시컬 스코프를 기억하는 함수.
  - 대표적인 예시로, 어떤 함수의 리턴으로 받게 되는 일급함수 중, 그 함수 외부에서 접근할 수 없는, 함수 내 지역변수에 접근할 수 있게 해주는 통로 역할을 하는 일급함수가 있다.
- 어떻게 이게 가능한 건가?
